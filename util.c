...
{
	const char *job_id, *prevhash, *coinb1, *coinb2, *version, *height, *nbits, *ntime;
	...
	height = json_string_value(json_array_get(params, 9));

	if (!job_id || !prevhash || !coinb1 || !coinb2 || !version || !height || !nbits || !ntime ||
	...
	hex2bin(sctx->job.height, height, 4);
